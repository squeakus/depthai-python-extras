...
import consts.resource_paths_fruits_ssdv1 as rscs
...
cmd_file = rscs.device_cmd_fpath
if args['dev_debug']:
    cmd_file = ''
    print('depthai will not load cmd file into device.')

labels = []
with open(rscs.blob_labels_fpath) as fp:
    labels = fp.readlines()
...
config = {
    ...
    'depth':
    {
        'calibration_file': rscs.calib_fpath,
        # 'type': 'median',
        'padding_factor': 0.3
    },
    'ai':
    {
        'blob_file': rscs.blob_fpath,
        'blob_file_config': rscs.blob_config_fpath,
