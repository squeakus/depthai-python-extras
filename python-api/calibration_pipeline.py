...
# streams_list = ['left', 'right', 'disparity', 'meta_d2h']
streams_list = ['meta_d2h', 'disparity', 'left', 'right']

pipieline = depthai.create_pipeline(
        streams = streams_list,
        cmd_file = consts.resource_paths.device_ai_cmd_fpath,
        pipeline_file_config = consts.resource_paths.pipeline_config_fpath,
        blob_file = consts.resource_paths.blob_fpath,
        blob_file_config = consts.resource_paths.blob_config_fpath,
        calibration_file = ''
        )

if pipieline is None:
    print('Pipeline is not created. Try reseting device ...')
    exit(1)
...
while run_capturing_images:
    print('tick')
    _, data_list = pipieline.get_available_nnet_and_data_packets()
